...
class RealTimeVoiceCog(commands.Cog):
    ...
    async def voice_join(self, interaction: discord.Interaction):
        ...
        if self._has_active_session(interaction.guild.id):
            await interaction.response.send_message("Already in a voice channel! Use `/voice_leave` first.", ephemeral=True)
            return
        ...
        try:
            ...
            await interaction.response.send_message(f"Joined {channel.name}! Use `/voice_start` to begin conversation.", ephemeral=True)
    ...
    async def voice_start(self, interaction: discord.Interaction, listen_to: discord.Member | None = None):
        ...
        if session.get("service") and session["service"].is_running:
            await interaction.response.send_message("Conversation already active! Use `/voice_stop` first.", ephemeral=True)
            return
    ...
    async def voice_stop(self, interaction: discord.Interaction):
        ...
        try:
            ...
            await interaction.response.send_message("Conversation stopped.", ephemeral=True)

        except Exception as e:
            self.logger.error(f"Error stopping conversation: {e}")
            await interaction.response.send_message(f"Error stopping conversation: {e}", ephemeral=True)
    ...
    async def voice_leave(self, interaction: discord.Interaction):
        ...
        try:
            ...
            await interaction.response.send_message(f"Error leaving voice channel: {e}", ephemeral=True)
